...
char *cursor_position(int row, int col);
void drawMenu(void);
...
void UART_send(UART_HandleTypeDef *huart, const char *string)
{
...
void drawMenu(void)
{
    ...
    // Print out the phrases at a certain point
    for (int i = 0; i < 4; i++)
    {
        // move cursor
        char *cursor = cursor_position(i + 1, 1);
        UART_send(&huart2, cursor);

        if (i == selected)
        {
            // Only print the cursor on the one that is selcted
            UART_send(&huart2, "> ");
        }
        else
        {
        ...
        // print the phrase now
        UART_send(&huart2, phrases[i]);
    }
}
...
int main(void)
{
    ...
    UART_send(&huart2, ClearScreen);
    // first thing is to print the phrases on the screen
    ...
    while (1)
    {
        ...
        {
            Command command1 = checkCommand();
            switch (command1)
            {
            case (COMMAND_UP):
            	// Make sure you wrap around
            	selected = (selected + 1) % 4;
            	drawMenu();
                break;

            case (COMMAND_DOWN):
                UART_send(&huart2, "COMMAND_DOWN");
            ...
            case (COMMAND_SELECT):
            	...
            	char* cursor = cursor_position(6,1);
            	UART_send(&huart2, cursor);

            	char buffer[60];
            	sprintf(buffer, "You chose: %s\n\r", phrases[selected]);
...
char *cursor_position(int row, int col)
{
    // Returns a pointer to the string for the command to be ran

    static char CursorPos[10];

    sprintf(CursorPos, "\x1B[%d;%dH", row, col);
